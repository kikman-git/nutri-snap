...
**Milestone 5 — onboarding + personalized targets — built + verified on simulator (2026-06-24).** First-run gate (`Features/Onboarding/`: `OnboardingView` + shared `ProfileForm` = `ProfileInputs`/`ProfileFieldsView`/`TargetPreviewCard`) collects body stats (sex/age/height/weight/activity/goal) → computes the personal daily target via **Mifflin–St Jeor** (`Models/NutritionMath.swift`: BMR → TDEE×activity×goal → 20/50/30 macro split, PRD §4). Micronutrient references are now **personalized by sex** (MHLW DRI; `NutritionMath.microReferences`) — the old hardcoded `Nutrient.referenceDaily` is gone, replaced by `MealStore.references` (a `NutrientAmounts`) threaded into the calendar bands, Trends sufficiency panel, day-diary `MicroBar`s, and the share card. `UserProfile` (`Models/NutritionModels.swift`) gained the body stats + an `onboarded` flag with a **tolerant `init(from:)`** so legacy M2/M3 profile docs decode (defaults) + get offered onboarding rather than crashing. `MealStore.apply(_:)` sets `target` + `references`; `saveProfile(...)`/`skipOnboarding()` persist offline-safe to `users/{uid}` via `setData(merge:)`. Onboarding is **skippable** (keeps the neutral default — gentle, no nag; PRD Q#3 resolved). Edit later via the **person icon in the Trends header → `ProfileSettingsSheet`**. `RootView` gates on `store.needsOnboarding` (a `fullScreenCover`). New screenshot hook: `FORCE_ONBOARDING=1` (pair with `USE_SAMPLE=1`) opens the gate headlessly. ⚠️ Caught + fixed a real **segmented-`Picker` binding bug** (an `Identifiable` `ForEach` derives a String `id`-tag that collides with the value `.tag`, so goal defaulted to *lose* not *maintain*) — fixed with `ForEach(…, id: \.self)`. **Verified on sim** (Mifflin math correct; default-refs no-regression). **Not yet verified live** (needs a device + registered App Check token): the Firestore profile write/persist + the gate firing for a genuine new user. Photo upload to Storage + Sign in with Apple (M4) still pending.
