...
- **UI revamp — "Warm Bloom" — ✅ ALL PHASES DONE (2026-06-26).** Phases 0–3 built + verified on the iPhone 16 Plus sim against the imported design (claude.ai/design `7dd99ae1…`, `Nutri Snap - Warm Bloom.dc.html`). The full design is now implemented: Theme foundation (fonts/colors/type scale/components), all 8 screen restyles, the Phase-2 viz (MicroBloom/ConicRing/sparkline), and the 3 net-new screens (Welcome, Today·Home, Fill the Gaps). The temp `GALLERY=1` gallery is removed. Full plan + status: **[`WARM_BLOOM_REVAMP.md`](WARM_BLOOM_REVAMP.md)**. ⚠️ Branch `warm-bloom-revamp` — **commits `83c8b97` (P1) · `03ceb66` (P3) · `3fb1ffd` (PRD) are local; not yet pushed past `83c8b97`.** Next real work: **deploy the M6 backend** to live-verify the scan path + D1/D2 (energy + the 4 new micros) end-to-end (see `M6_SETUP.md`).
